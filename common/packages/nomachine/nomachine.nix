  ...
  config = lib.mkIf cfg.enable {
    ...
    services.nxserver = {
      ...
      nodeSettings = {
        ...
        DefaultDesktopCommand = "/run/current-system/sw/bin/dbus-launch --exit-with-session /run/current-system/sw/bin/startplasma-x11";
      };
    };
    ...
    environment.etc = {
      ...
      "NX/server/localhost/player.cfg" = {
        text = ''PlayerRoot = "${cfg.package}/NX"'';
        mode = "644";
      };

      "NX/server/localhost/runner.cfg" = {
        text = ''RunnerRoot = "${cfg.package}/NX"'';
        mode = "644";
      };
